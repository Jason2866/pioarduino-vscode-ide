...
export async function findPythonExecutable() {
  // First try local check without network
  const localPython = await findPythonExecutableLocal();
  if (localPython) {
    return localPython;
  }
    ...
    for (const exename of exenames) {
      ...
      try {
        if (fs.existsSync(executable)) {
          // Basic local check without network call
          return executable;
        }
      } catch (err) {
...
export async function findPythonExecutableNetwork() {
  const exenames = proc.IS_WINDOWS ? ['python.exe'] : ['python3', 'python'];
  const envPath = process.env.PLATFORMIO_PATH || process.env.PATH;
  const errors = [];
  for (const location of envPath.split(path.delimiter)) {
