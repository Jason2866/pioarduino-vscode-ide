...
export async function ensureCompileCommands(projectDir, observer, envDir) {
  ...
  // Check the processed clangd copy first, then the build-system originals.
  const candidates = [
    path.join(projectDir, '.pio', 'clangd', 'compile_commands.json'),
...
export async function fixupCompileCommands(projectDir, envDir) {
  ...
  // Read the original compile_commands.json – try project root first (PIO
  // compiledb output), then the env build dir (CMake / ninja output for
  // Arduino-as-component / ESP-IDF projects).  The original is never modified.
  ...
  const clangdDir = path.join(projectDir, '.pio', 'clangd');
...
export async function ensureClangdArgs(projectDir) {
  ...
  // in .pio/clangd/ (written by fixupCompileCommands) so the original database
  // generated by the build system is never consumed directly.
  const ccDir = path.join(projectDir, '.pio', 'clangd');
