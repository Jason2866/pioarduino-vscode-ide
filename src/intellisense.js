...
export async function ensureCompileCommands(projectDir, observer, envDir) {
  ...
  const clangdPath = path.join(projectDir, '.pio', 'clangd', 'compile_commands.json');
  try {
...
export async function fixupCompileCommands(projectDir, envDir) {
  ...
  // Read the original compile_commands.json – try the project root first
  // (PIO compiledb output), then fall back to env build dir
  // (CMake / ninja output for Arduino-as-component / ESP-IDF projects).
